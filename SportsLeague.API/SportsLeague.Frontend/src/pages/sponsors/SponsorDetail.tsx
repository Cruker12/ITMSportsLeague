...
export default function SponsorDetail() {
  ...
  const handleAddTournament = async () => {
    ...
    try {
      await sponsorApi.addTournament(parseInt(id), {
  ...
  const columns = [
    ...
    { key: 'joinedAt', header: 'Fecha', render: (item: TournamentSponsorResponse) => formatDateTime(item.joinedAt) },
    {
      key: 'actions',
      header: '',
      render: (item: TournamentSponsorResponse) => (
        <button
          className="btn btn-sm btn-danger"
          onClick={() => handleRemoveTournament(item.tournamentId)}
        >
          X
        </button>
      ),
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <button className="btn btn-sm" onClick={() => navigate('/sponsors')}>
            ← Volver
          </button>
          <h1>{sponsor.name}</h1>
        </div>
      ...
      <div className="detail-grid">
        <div className="detail-card">
          ...
          <p>
            ...
            <StatusBadge type="sponsor" value={sponsor.category} />
          </p>
        </div>
      </div>
            ...
            <div className="modal-body">
              ...
              <div className="form-actions">
                ...
                <button
                  ...
                  disabled={!selectedTournamentId || contractAmount <= 0}
                >
                  Vincular
                </button>
              </div>
