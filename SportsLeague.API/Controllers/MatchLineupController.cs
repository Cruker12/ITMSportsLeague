...
public class MatchLineupController : ControllerBase
{
        ...
        int matchId, [FromBody] CreateMatchLineupDto dto)
    {
        try
        {
            ...
            // Re-consultar con includes para que AutoMapper resuelva PlayerName y TeamName
            var result = await _lineupService.GetLineupByMatchAsync(matchId);
            var createdDto = _mapper.Map<MatchLineupDto>(
                result.FirstOrDefault(l => l.Id == created.Id));

            return CreatedAtAction(nameof(GetLineup),
                new { matchId }, createdDto);
        }
